...
.top-nav-spacer { width: 40px; flex-shrink: 0; }
.top-nav-title {
    font-family: var(--font-sans) !important;
    font-size: 14px;
    font-weight: 600;
...
.top-nav-title em { font-style: normal; color: var(--accent) !important; }
.top-nav-tags {
    display: flex;
    gap: 6px;
    margin-left: auto;
...
.stat-value.c-sky   { color: #38BDF8 !important; }
...
/* ── FIX: Hamburger button override — only target the menu button ── */
#fundiq-menu-btn .stButton > button {
    background: var(--bg-card) !important;
    color: var(--text-primary) !important;
    border: 1px solid var(--border) !important;
...
#fundiq-menu-btn .stButton > button:hover {
    border-color: var(--accent) !important;
    background: var(--bg-elevated) !important;
...
/* ══════════════════════════════════════
   FIX: Chart container — prevent plotly overflow/clip
══════════════════════════════════════ */
...
# FIX 1: Gauge — increased height to 340, more generous margins so arc is never clipped
def make_gauge(value, title=""):
    ...
    # FIX: height=340, generous top/side margins so half-circle doesn't get clipped
    fig.update_layout(
        height=340,
...
def make_radar(values, name, color_hex):
    ...
    fig.update_layout(
        polar=dict(
            ...
            # FIX: Angular axis labels — pure white (#FFFFFF)
            angularaxis=dict(tickfont=dict(color="#FFFFFF", size=12, family="DM Sans", weight="bold"),
                gridcolor="rgba(255,255,255,0.10)", linecolor="rgba(255,255,255,0.10)"),
        ),
        showlegend=False, height=340,
        margin=dict(t=20, b=20, l=55, r=55),
...
def make_forecast_bar(current, projected):
    ...
    fig.add_trace(go.Bar(
        ...
        y=[current, projected],
        marker=dict(color=["#6B7280", "#22C55E"], line=dict(color=["#6B7280", "#22C55E"], width=0), cornerradius=4),
...
def make_compare_bar(a_name, b_name, a_vals, b_vals, metrics):
    ...
    for name, vals, color in [(a_name, a_vals, C_GOLD), (b_name, b_vals, C_NEUTRAL)]:
        fig.add_trace(go.Bar(
            ...
            marker=dict(color=color, line=dict(color=color, width=0), cornerradius=3),
    ...
    fig.update_layout(
        polar=dict(
            ...
            radialaxis=dict(visible=True, range=[0,100],
                ...
                tickvals=[0,25,50,75,100], ticktext=["0","25","50","75","100"]),
            # FIX: Angular axis labels white for compare radar too
            angularaxis=dict(tickfont=dict(color="#FFFFFF", size=11, family="DM Sans", weight="bold"),
                gridcolor="rgba(255,255,255,0.08)", linecolor="rgba(255,255,255,0.08)"),
        ),
        ...
        height=325, margin=dict(t=12, b=12, l=48, r=96), **_LAYOUT_BASE,
    )
    return fig


# ─── Core Logic ────────────────────────────────────────────────────────────────
def compute(funding, rounds, inv_rounds, age, miles, rels):
    eff  = funding / (rounds + 1)
    pyr  = funding / (age + 1)
    log  = np.log1p(funding)
    raw  = 0.4 * np.log1p(eff) + 0.3 * np.log1p(pyr) + 0.3 * log
    hs   = min(raw / 10, 1.0) * 100
    X    = np.array([[eff, pyr, log, miles, rels, inv_rounds, age, rounds]])
    base = float(pipeline.predict_proba(X)[0][1])
    prob = 0.7 * base + 0.3 * (hs / 100)
    radar_vals = [
        max(min(np.log1p(funding)/20, 1)*100, 3),
        max(min(rounds/10, 1)*100, 3),
        max(min(age/15, 1)*100, 3),
        max(min(miles/10, 1)*100, 3),
        max(min(rels/20, 1)*100, 3),
    ]
    return prob, hs, base, eff, pyr, log, radar_vals

def get_recommendations(funding, rounds, miles, rels, age):
    recs = []
    if funding == 0:
        recs.append(("HIGH","No funding detected","Securing seed funding is the most critical first step for startup survival and investor credibility."))
    elif funding < 100_000:
        recs.append(("MED","Funding below $100K","Target angel investors or accelerators to reach meaningful seed-stage funding."))
    elif funding < 1_000_000:
        recs.append(("MED","Pre-Series A range","Strong momentum — prepare your pitch deck for Series A to unlock institutional capital."))
    else:
        recs.append(("OK","Funding level healthy","Maintain investor relations and deploy capital efficiently to maximize runway."))
    if rounds == 0:
        recs.append(("HIGH","No funding rounds completed","Initiating your first round signals investor confidence and unlocks future rounds."))
    elif rounds < 3:
        recs.append(("MED","Limited funding rounds","Additional rounds would significantly increase investor trust and valuation."))
    else:
        recs.append(("OK","Multiple rounds completed","Strong investor conviction — focus on deploying capital for maximum growth impact."))
    if miles == 0:
        recs.append(("HIGH","No milestones achieved","Product and market milestones are critical proof points for investor diligence."))
    elif miles < 3:
        recs.append(("MED","Few milestones recorded","Accelerate product launches and market milestones to demonstrate traction."))
    else:
        recs.append(("OK","Strong milestone record","Excellent execution track record — use milestones as your primary investor narrative."))
    if rels < 2:
        recs.append(("HIGH","Weak investor network","Expand investor and partner network urgently — relationships drive deal flow and introductions."))
    elif rels < 5:
        recs.append(("MED","Moderate network","Invest in relationship building — warm introductions convert 3× better than cold outreach."))
    else:
        recs.append(("OK","Strong network","Leverage your network for the next growth phase — introductions, partnerships, talent."))
    if age < 2:
        recs.append(("MED","Early-stage startup","Focus ruthlessly on product-market fit before scaling spend or team size."))
    elif age >= 7:
        recs.append(("OK","Proven longevity","7+ years of survival demonstrates market adaptability — highlight this in investor conversations."))
    else:
        recs.append(("OK","Established startup","Solid trajectory — focus on scaling what's working and eliminating what's not."))
    return recs


# ══════════════════════════════════════════════════════
# NATIVE STREAMLIT SIDEBAR — Fully restyled with CSS
# This is the only approach that works reliably in Streamlit's iframe
# ══════════════════════════════════════════════════════

# Populate the native sidebar
with st.sidebar:
    st.markdown("""
<style>
/* ── Override sidebar interior colors ── */
...
<div style="padding:12px 20px;border-bottom:1px solid #1F2937;">
  <div style="display:inline-flex;align-items:center;gap:7px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);border-radius:50px;padding:4px 11px 4px 9px;">
    <span style="width:6px;height:6px;border-radius:50%;background:#22C55E;display:inline-block;animation:sbpulse 3s ease-in-out infinite;"></span>
...
<div style="padding:0 20px;">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid rgba(31,41,55,0.7);"><span style="font-size:9.5px;color:#9CA3AF;font-family:'DM Mono',monospace;">Features</span><span style="font-size:9.5px;font-weight:500;color:#F9FAFB;font-family:'DM Mono',monospace;">8 signals</span></div>
...
<div style="font-size:7.5px;letter-spacing:2.5px;text-transform:uppercase;color:#9CA3AF;padding:18px 20px 8px;display:block;font-family:'DM Mono',monospace;">Input Features</div>
<div style="padding:0 20px 16px;">
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Funding Efficiency</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Funding per Year</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Log Funding</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Milestones Achieved</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Relationships</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Investment Rounds</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Startup Age</div>
  <div style="display:flex;align-items:center;gap:9px;padding:5px 0;font-size:9.5px;color:#F9FAFB;font-family:'DM Mono',monospace;"><div style="width:14px;height:14px;border-radius:3px;background:rgba(34,197,94,0.08);border:1px solid rgba(34,197,94,0.18);display:flex;align-items:center;justify-content:center;font-size:8px;color:#22C55E;flex-shrink:0;">&#10003;</div>Funding Rounds</div>
</div>
...
<div class="page-header">
  <div>
    <div class="page-eyebrow">Startup Intelligence Platform</div>
    <div class="page-title">Funding <em>Analysis</em></div>
    <div class="page-desc">CatBoost · 8-signal engine · 462K+ startups analyzed · ROC-AUC 0.965</div>
  </div>
...
with tab1:
    ...
    with c1: total_funding  = st.number_input("Total Funding ($)", 0.0, 1e10, step=10000.0, format="%.0f")
    with c2: funding_rounds = st.number_input("Funding Rounds", min_value=0, max_value=50)
    with c3: inv_rounds     = st.number_input("Investment Rounds", min_value=0, max_value=50)
    c4, c5, c6 = st.columns(3)
    with c4: startup_age   = st.slider("Startup Age (years)", 0, 30, 3)
    with c5: milestones    = st.number_input("Milestones Achieved", min_value=0, max_value=100)
    with c6: relationships = st.number_input("Relationships / Investors", min_value=0, max_value=500)

    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("Run Analysis", use_container_width=True):
        prob, hs, base_prob, eff, pyr, log_f, radar_vals = compute(
        ...
        <div class="card-grid-4">
          <div class="stat-card {borders[0]}">
            ...
            <div class="stat-sub">Model + Health Composite</div>
          </div>
        ...
        # FIX 3: Equal columns + use_container_width=True on both charts
        # Gauge needs vertical breathing room — stacked layout on narrow screens works better
        ch1, ch2 = st.columns(2)
        with ch1:
            st.plotly_chart(
                make_gauge(round(prob_pct, 1), "SUCCESS PROBABILITY"),
                use_container_width=True,
                config={"displayModeBar": False},
            )
        with ch2:
            st.plotly_chart(
                make_radar(radar_vals, "Startup", C_GOLD),
                use_container_width=True,
                config={"displayModeBar": False},
            )
        ...
        st.plotly_chart(make_forecast_bar(prob_pct, f_prob*100), use_container_width=True, config={"displayModeBar": False})
        ...
        if total_funding == 0:
            st.warning("Zero funding detected — predictions may be less reliable for unfunded startups.")
...
with tab2:
    ...
    with col_a:
        ...
        a_fund   = st.number_input("Total Funding ($)", 0.0, 1e10, step=10000.0, key="af", format="%.0f")
        a_rounds = st.number_input("Funding Rounds", 0, 50, key="ar")
        a_inv    = st.number_input("Investment Rounds", 0, 50, key="ai")
        a_age    = st.slider("Age (years)", 0, 30, 3, key="aa")
        a_miles  = st.number_input("Milestones", 0, 100, key="am")
        a_rels   = st.number_input("Relationships", 0, 500, key="arel")
    ...
    with col_b:
        ...
        b_fund   = st.number_input("Total Funding ($)", 0.0, 1e10, step=10000.0, key="bf", format="%.0f")
        b_rounds = st.number_input("Funding Rounds", 0, 50, key="br")
        b_inv    = st.number_input("Investment Rounds", 0, 50, key="bi")
        b_age    = st.slider("Age (years)", 0, 30, 3, key="ba")
        b_miles  = st.number_input("Milestones", 0, 100, key="bm")
        b_rels   = st.number_input("Relationships", 0, 500, key="brel")

    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("Compare Startups", use_container_width=True):
        a_prob, a_hs, _, _, _, a_log_f, a_radar = compute(a_fund, a_rounds, a_inv, a_age, a_miles, a_rels)
        b_prob, b_hs, _, _, _, b_log_f, b_radar = compute(b_fund, b_rounds, b_inv, b_age, b_miles, b_rels)
        ...
        # FIX 4: Compare gauges — each in its own column, full width, no clipping
        ...
        with ga:
            ...
            st.plotly_chart(
                make_gauge(round(a_prob*100, 1), a_name.upper()),
                use_container_width=True,
                config={"displayModeBar": False},
            )
        with gb:
            ...
            st.plotly_chart(
                make_gauge(round(b_prob*100, 1), b_name.upper()),
                use_container_width=True,
                config={"displayModeBar": False},
            )
...
<div class="page-footer">
  <span>FundIQ · Startup Health Intelligence System · Built by Nischal Mahajan</span>
  <span>CatBoost · ROC-AUC 0.965 · <a href="https://github.com/Nischal-Mahajan/Startup-Health-Intelligence-System">GitHub ↗</a></span>
</div>
""", unsafe_allow_html=True)